...
class T_CNN(object):
  ...
  def train(self, config):
    ...
    #expand_test1 = expand_test1.squeeze()
    ...
    #batch_test_depth= tf.expand_dims(batch_test_depth1, 1, name=None)
    #batch_test_depth = batch_test_depth.squeeze()
    # import pdb
    # pdb.set_trace()
    ...
    # import pdb
    # pdb.set_trace()
      # result_c = self.pred_c.eval({self.images: batch_test_image})
      # result_f = self.pred_f.eval({self.images: batch_test_image})
      # result_h = self.pred_h.eval({self.images: batch_test_image})

    _,h ,w , c = result_h.shape
    for id in range(0,1):
        #result_t0 = result_t[id,:,:,:].reshape(h , w , 1)

        #result_x010 = result_x01[id,:,:,:].reshape(h , w , 3)
        #result_x020 = result_x02[id,:,:,:].reshape(h , w , 3)
        #result_x030 = result_x03[id,:,:,:].reshape(h , w , 3)
        result_h0 = result_h[id,:,:,:].reshape(h , w , 3)
        result_h0 = result_h0.squeeze()

        #result_h01 = result_h1[id,:,:,:].reshape(h , w , 1)
        #result_h01 = result_h01.squeeze()

        #image_path0 = os.path.join(os.getcwd(), config.sample_dir)
        #image_path = os.path.join(image_path0, "%2dtest_image_t.png"%(self.id))
        #imsave_lable(result_t0, image_path)



        #dehaze = (result_h0+1.)/2 
        #image_path = os.path.join(image_path0, "%2dtest_dehaze.png"%(self.id))
        #imsave_lable(dehaze, image_path)

        image_path0 = os.path.join(os.getcwd(), config.sample_dir)
        image_path = os.path.join(image_path0, self.test_image_name)
        #image_path1 = os.path.join(image_path0, self.test_image_name+'_out.png')
        #imsave_lable((result_h0+1.)/2, image_path)
        imsave_lable(result_h0, image_path)
        #imsave_lable(result_h01, image_path1)
  ...
  def model(self):
    ...
    with tf.variable_scope("main_branch") as scope:      
      ...
      #conb00 = tf.concat(axis = 3, values = [self.images,temp]) 
      #conv_wb9 = tf.nn.relu(conv2d(conb00, 3,32, k_h=7, k_w=7, d_h=1, d_w=1,name="conv2wb_9"))
      #conv_wb10 = tf.nn.relu(conv2d(conv_wb9, 32,32, k_h=5, k_w=5, d_h=1, d_w=1,name="conv2wb_10"))
      #conv_wb11 =tf.nn.relu(conv2d(conv_wb10, 32,32, k_h=3, k_w=3, d_h=1, d_w=1,name="conv2wb_11"))
      #conv_wb12 = tf.nn.relu(conv2d(conv_wb11, 32,32, k_h=5, k_w=5, d_h=1, d_w=1,name="conv2wb_12"))
      #conv_wb13 =tf.nn.relu(conv2d(conv_wb12, 32,32, k_h=3, k_w=3, d_h=1, d_w=1,name="conv2wb_13"))
      #output1 =conv2d(conv_wb13, 32,3, k_h=3, k_w=3, d_h=1, d_w=1,name="conv2wb_15")
